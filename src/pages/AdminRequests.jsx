...
const AdminRequests = () => {
  ...
  const [confirmDialog, setConfirmDialog] = useState({ open: false, requestId: null, status: '' });
  ...
  const handleUpdateStatus = (requestId, newStatus) => {
    setConfirmDialog({ open: true, requestId, status: newStatus });
  };

  const executeUpdateStatus = async () => {
    const { requestId, status: newStatus } = confirmDialog;
    setConfirmDialog({ open: false, requestId: null, status: '' });
    try {
  ...
  return (
    <div className="space-y-6 mx-auto">
            ...
            >
              <div className="flex justify-between items-center px-8 py-5 border-b border-slate-100 dark:border-dark-border bg-slate-50/80 dark:bg-dark-bg/50 shrink-0">
                ...
                <div className="flex items-center space-x-4">
                  <span className={`px-3 py-1 rounded-lg border text-xs font-bold tracking-wide uppercase ${getStatusColor(selectedRequest.status)}`}>{selectedRequest.status}</span>
      ...
      <ConfirmDialog
        ...
        title={confirmDialog.status === 'REJECTED' ? 'Reject this request?' : confirmDialog.status === 'APPROVED' ? 'Approve Request?' : 'Mark Request Completed?'}
        message={
          confirmDialog.status === 'REJECTED'
            ...
            : confirmDialog.status === 'APPROVED'
              ? 'This will approve the production request and notify the buyer.'
              : 'This will mark the production request as completed, indicating that custom weaving is finished.'
        }
        confirmText={confirmDialog.status === 'REJECTED' ? 'Yes, Reject' : confirmDialog.status === 'APPROVED' ? 'Yes, Approve' : 'Yes, Complete'}
        cancelText="Go Back"
