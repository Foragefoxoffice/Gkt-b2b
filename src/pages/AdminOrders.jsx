...
const AdminOrders = () => {
  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-dark-bg border-b border-slate-200 dark:border-dark-border">
                  ...
                  <th className="p-4 font-semibold text-slate-600 dark:text-slate-300">Buyer</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-dark-border">
                {orders.map(order => (
                  <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-dark-bg/50 transition-colors">
                    ...
                    <td className="p-4 text-slate-600 dark:text-slate-400">{order.buyer?.name}</td>
                    ...
                    <td className="p-4 flex justify-end space-x-2">
                      <button onClick={() => handleViewOrder(order.id)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View / Approve">
                        <Eye size={18} />
                      </button>
                    </td>
                  </tr>
      ...
      {selectedOrder && (
        <div className="fixed modal_main inset-0 bg-slate-900/40 backdrop-blur-sm mt-0 flex items-center justify-center z-50 p-4 transition-all">
          <div className="bg-white dark:bg-dark-card rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden border border-slate-100 dark:border-dark-border flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="grid grid-cols-2 gap-4 text-sm bg-slate-50/50 dark:bg-dark-bg/30 p-5 rounded-xl border border-slate-100 dark:border-dark-border">
                <div>
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Buyer</p>
                  <p className="font-semibold text-slate-800 dark:text-white">{selectedOrder.buyer?.name} ({selectedOrder.buyer?.code})</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Current Status</p>
                ...
                <div className="col-span-2 mt-2">
                  ...
                  <p className="font-semibold text-slate-800 dark:text-white bg-white dark:bg-dark-card p-3 rounded-lg border border-slate-200 dark:border-dark-border">{selectedOrder.remarks || 'None'}</p>
                </div>
                {selectedOrder.approvals && selectedOrder.approvals.length > 0 && selectedOrder.approvals[0].remarks && (
                  <div className="col-span-2 mt-2 pt-2 border-t border-slate-200 dark:border-dark-border">
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Admin Remarks</p>
                    <p className="font-semibold text-slate-800 dark:text-white bg-primary-50 dark:bg-primary-900/10 p-3 rounded-lg border border-primary-100 dark:border-primary-900/30 text-primary-800 dark:text-primary-300">{selectedOrder.approvals[0].remarks}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                  ...
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 dark:bg-dark-bg border-b border-slate-200 dark:border-dark-border">
                      <tr>
                        ...
                        <th className="p-4 font-semibold text-slate-600 dark:text-slate-300">Req. Qty</th>
                        <th className="p-4 font-semibold text-slate-600 dark:text-slate-300">Avail. Stock</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-dark-border">
                      {selectedOrder.items.map(item => (
                        <tr key={item.id} className={`${item.quantity > item.design.availableStock ? 'bg-red-50/50 dark:bg-red-900/10' : 'hover:bg-slate-50/50 dark:hover:bg-dark-bg/50 transition-colors'}`}>
                          <td className="p-4">
                          ...
                          <td className={`p-4 font-bold ${item.quantity > item.design.availableStock ? 'text-red-600' : 'text-green-600'}`}>
                            {item.design.availableStock}
                          </td>
                ...
                {selectedOrder.items.some(i => i.quantity > i.design.availableStock) && (
                  <div className="mt-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-3 rounded-lg border border-red-100 dark:border-red-900/50 flex items-start">
                    <XCircle size={18} className="mr-2 mt-0.5 shrink-0" />
                    <p className="text-sm font-medium">Warning: Some items request more quantity than is available in stock. Proceed with caution.</p>
                  </div>
                )}
              </div>
            ...
            <div className="flex justify-between items-center px-6 py-4 mt-auto border-t border-slate-100 dark:border-dark-border bg-slate-50/50 dark:bg-dark-bg/20 shrink-0">
              ...
              {selectedOrder.status === 'PENDING' && (
                <div className="flex space-x-3">
                  <button onClick={() => handleUpdateStatus(selectedOrder.id, 'CANCELLED')} className="px-5 py-2.5 text-sm font-medium bg-white border border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 rounded-lg shadow-sm transition-all flex items-center">
                    <XCircle size={16} className="mr-2" /> Reject
                  </button>
                  <button onClick={() => handleUpdateStatus(selectedOrder.id, 'APPROVED')} className="px-5 py-2.5 text-sm font-medium text-white bg-green-600 hover:bg-green-500 rounded-lg shadow-sm shadow-green-600/30 transition-all flex items-center">
                    <CheckCircle size={16} className="mr-2" /> Approve & Reserve Stock
                  </button>
                </div>
              )}
            </div>
